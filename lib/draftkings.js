...
function extractMarketsByEvent(eventGroup) {
    ...
    const allCategories = eventGroup.offerCategories || [];
    // DK lists alternate spread/total lines (e.g. "Alternate Run Lines") in
    // their own category, separate from the main "Game Lines" category. Both
    // can match our label substrings below, and since a later match
    // overwrites an earlier one, processing alternates would silently return
    // the wrong line. Prefer the main category when we can identify it;
    // fall back to scanning everything if DK's category naming has changed.
    const gameLinesCategories = allCategories.filter(c => /game lines/i.test(c.name || ''));
    const categories = gameLinesCategories.length ? gameLinesCategories : allCategories;
